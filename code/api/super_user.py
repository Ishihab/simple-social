...
async def superuser_only_route(current_user: current_super_user):
    if not current_user:
        raise HTTPException(
            status_code=403, detail="You must be a superuser to access this route."
        )
...
async def get_all_users(
    current_user: current_super_user, session: session, cursor_params: cursor_params
):
    if not current_user:
        logger.warning(
            f"Unauthorized access attempt to superuser route by user {current_user.id if current_user else 'unknown'}."
        )
        raise HTTPException(
            status_code=403, detail="You must be a superuser to access this route."
        )
