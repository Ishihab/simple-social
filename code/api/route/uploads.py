...
async def get_presigned_upload_url(
    ...
    try:
        ...
        logger.error("Error creating object store entry", file_key=unique_file_key)
        raise HTTPException(
            status_code=500, detail="Could not create object store entry"
        )
    logger.info(
        "Object store entry created successfully",
        file_key=unique_file_key,
        object_id=str(object_store_object.id),
    )
    logger.info("Presigned URL generated successfully", file_key=unique_file_key)
