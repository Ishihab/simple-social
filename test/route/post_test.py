...
async def test_post_create_and_delete(authed_normal_user_client, authed_user_2_client, authed_user_post):
    post_data = {
    ...
    faild_delete_response = await authed_normal_user_client.delete(f"/posts/{second_user_post_id}")
    assert faild_delete_response.status_code == 404

    first_user_post_id = authed_user_post["post_id"]
    delete_response = await authed_normal_user_client.delete(f"/posts/{first_user_post_id}")
    assert delete_response.status_code == 200

async def test_post_like_and_unlike(authed_normal_user_client, authed_user_2_client, authed_user_post):
    like_response = await authed_user_2_client.post(f"/posts/{authed_user_post['post_id']}/like")
    ...
    unlike_response = await authed_user_2_client.post(f"/posts/{authed_user_post['post_id']}/like")
...
async def test_feed_contains_post(authed_normal_user_client, authed_user_2_client, authed_user_post):
