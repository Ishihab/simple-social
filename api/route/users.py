...
from api.dependency import session, current_active_user, current_active_user_optional
...
async def get_user_profile(
    ...
    try:
        ...
        current_user_followers: CursorPage[schemas.UserRead] = await crud.get_followers(session, current_user.id, CursorParams())
        current_user_following: CursorPage[schemas.UserRead] = await crud.get_following(session, current_user.id, CursorParams())
        user_posts: CursorPage[schemas.PostFeedRead] = await crud.get_user_posts(session, user_id, CursorParams())
        liked_post_ids = await crud.get_liked_post_ids(session, current_user.id, [post.id for post in user_posts.items])
    ...
    logger.info(f"Fetched user profile", profile_user_id=str(user_id))
    return templates.TemplateResponse(
        request=request,
...
async def follow_user(
    ...
    try:
        is_self, is_following = await crud.follow_user(session, followee_id=user_id, follower_id=current_user.id)
    ...
    if is_self:
        logger.info(f"User {current_user.id} attempted to follow themselves.")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You cannot follow yourself.")
    logger.info(f"User {current_user.id} followed user {user_id}.", follower_id=str(current_user.id), followee_id=str(user_id))
    return templates.TemplateResponse(
        request=request,
...
async def get_followers(
    ...
    try:
        followers: CursorPage[schemas.UserRead] = await crud.get_followers(session, user_id, cursor)
    except DatabaseReadError as e:
...
async def get_following(
    ...
    try:
        following: CursorPage[schemas.UserRead] = await crud.get_following(session, user_id, cursor)
    except DatabaseReadError as e:
