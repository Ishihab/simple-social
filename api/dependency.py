...
async def get_access_token_db(session: Annotated[AsyncSession, Depends(get_async_session)]):
...
class UserManager(UUIDIDMixin, BaseUserManager[User, UUID]):
    ...
    async def on_after_forgot_password(self, user: User, token: str, request: Request | None = None):
        logger.info(f"User {user.id} has requested a password reset token.", user_id=str(user.id))
...
def get_database_strategy(access_token_db: Annotated[AccessTokenDatabase[AccessToken], Depends(get_access_token_db)]):
...
non_binded_current_active_user_optional = fastapi_users.current_user(active=True, optional=True)
non_binded_current_super_user = fastapi_users.current_user(active=True, superuser=True)


async def get_current_active_user(user: Annotated[User, Depends(non_binded_current_active_user)]):
    structlog.contextvars.bind_contextvars(user_id=str(user.id))
    return user

async def get_current_active_user_optional(user: Annotated[User | None, Depends(non_binded_current_active_user_optional)]):
    if user:
        structlog.contextvars.bind_contextvars(user_id=str(user.id))
    return user

async def get_current_super_user(user: Annotated[User, Depends(non_binded_current_super_user)]):
    structlog.contextvars.bind_contextvars(user_id=str(user.id))
    return user

current_active_user = Annotated[User, Depends(get_current_active_user)]
current_active_user_optional = Annotated[User | None, Depends(get_current_active_user_optional)]
