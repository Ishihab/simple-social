...
async def delete_orphaned_objects(session: AsyncSession):
    ...
    if orphaned_keys:
        ...
        for i in range(0, len(orphaned_keys), max_batch_size):
            ...
            deleted = await run_in_threadpool(delete_objects_from_s3, objects=batch_keys)
            if deleted:
                logger.info(f"Deleted {len(deleted)} orphaned objects from S3.", deleted_count=len(batch_keys))
                try:
                    await remove_orphaned_objects(session, [key['Key'] for key in batch_keys])
...
async def delete_orphaned_objects_util():
    try:
        ...
        print(f"Error deleting orphaned objects: {e}")
